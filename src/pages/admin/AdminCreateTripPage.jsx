  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const formattedData = {
        ...
        sub_expenses: data.sub_expenses.map(exp => ({
          ...exp,
          expense_amount: parseFloat(exp.expense_amount)
        })),
        images: images.map(image => URL.createObjectURL(image)) // This is temporary, you'll need to handle image upload separately
      };

      const response = await axios.post('http://localhost:3000/api/trips/', formattedData);
      ...
      <div className="container mx-auto px-4 max-w-4xl">
        <div className="bg-white rounded-lg shadow-xl overflow-hidden">
          <div className="bg-blue-600 p-6">
            ...
            <p className="text-blue-100 mt-2">Fill in the details to create an exciting new trip!</p>
          </div>
          <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="form-control">
                <label htmlFor="duration_days" className="label text-gray-700 font-semibold">Duration (days)</label>
                <input
                  id="duration_days"
                  type="number"
                  className={`input input-bordered w-full ${errors.duration_days ? 'input-error' : ''}`}
                  {...register("duration_days", { required: "Duration is required", min: 1 })}
                  placeholder="Enter duration in days"
                />
                {errors.duration_days && <span className="text-red-500 text-sm mt-1">{errors.duration_days.message}</span>}
              </div>
              <div className="form-control">
            ...
            <div className="form-control">
              <label htmlFor="images" className="label text-gray-700 font-semibold">Upload Images</label>
              <input
                id="images"
                type="file"
                multiple
